...
public class GameController implements ActionListener {
    ...
    public GameController(BoardPanel b,SidePanel s,EvalBar eb,JMenuBar mb){
        boardPanel = b;
        sidePanel = s;
        menuBar = mb;
        evalBar = eb;
    ...
    public void updateEvalBar(){
        evalBar.playerIsWhite = playerIsWhite;
        evalBar.repaint();
    }
    ...
    public void actionPerformed(ActionEvent actionEvent) {
        ...
        if(promotionWindow.isVisible()){
            ...
            if(src == sidePanel.prevButton && onMoveIndex>0){
                ...
                updateEvalBar();
                return;
            }else if(src == sidePanel.startButton && onMoveIndex != 0){
                for(;onMoveIndex>0;onMoveIndex--){
                    engine.undoMove(movesMade.get(onMoveIndex-1));
                }
                boardPanel.render(engine.cb.board,!playerIsWhite);
                updateEvalBar();
                return;
            }
            else if(src == sidePanel.endButton && onMoveIndex != movesMade.size()){
                for(;onMoveIndex<movesMade.size();onMoveIndex++){
                    engine.makeMove(movesMade.get(onMoveIndex));
                }
                boardPanel.render(engine.cb.board,!playerIsWhite);
                updateEvalBar();
                return;
